-- 181_nextsteps_phases.sql
--
-- (Renumbered from 180: the Care Protocol work took 180 in parallel. This
-- migration is order-independent relative to it, and it is idempotent, so
-- re-running it under the new number is harmless if 180 already ran.)
--
...
-- current phases or zero.
--
-- SECURITY INVOKER means RLS applies to whoever calls them. Note that the API
-- calls them with the SERVICE key, which bypasses RLS, so for that caller these
-- are NOT a security boundary: api/nextsteps-route.js does its own ownership
-- check before both, and that check is the boundary. INVOKER is here so that a
-- future caller using the anon key (a direct .rpc() from the browser) is
-- covered by the policy in section 6 rather than silently privileged.
