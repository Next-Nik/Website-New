...
function selfWheelBase(svg, selfData) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    html += `<circle cx="${tx}" cy="${ty}" r="3" fill="rgba(76,107,69,0.5)"/>`
  }
  for (let i = 0; i < n; i++) {
    const a = angleFor(i)
...
function civWheelBase(svg, civData) {
  ...
  ;[0.33, 0.66].forEach(f => {
    ...
    html += `<polygon points="${pts.join(' ')}" fill="none" stroke="rgba(76,107,69,0.18)" stroke-width="1"/>`
  })
  for (let i = 0; i < n; i++) {
    const a = angleFor(i)
    const tx = cx + maxR * Math.cos(a), ty = cy + maxR * Math.sin(a)
...
function applyScaleZoom(svg, cx, cy, maxR, timers, civData) {
  ...
  rings.forEach(ring => {
    ...
    const stroke = ring.lit ? 'var(--gold)' : 'rgba(76,107,69,0.32)'
