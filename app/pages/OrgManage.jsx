      ...
      {DOMAIN_LIST.map(d => {
        ...
        return (
          <button key={d.value} type="button" onClick={() => toggle(d.value)}
            style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', padding: '6px 14px', borderRadius: '40px', cursor: 'pointer', border: on ? '1.5px solid rgba(200,146,42,0.78)' : '1.5px solid rgba(200,146,42,0.25)', background: on ? 'rgba(200,146,42,0.10)' : 'transparent', color: on ? gold : 'rgba(15,21,35,0.55)', transition: 'all 0.15s' }}>
            {d.label}
          </button>
        )
      })}
  ...
  return (
    <div style={{ background: offering.is_flagship ? 'rgba(200,146,42,0.05)' : '#FFFFFF', border: offering.is_flagship ? '1.5px solid rgba(200,146,42,0.78)' : '1.5px solid rgba(200,146,42,0.18)', borderRadius: '14px', padding: '22px 26px', marginBottom: '12px', position: 'relative' }}>
      {offering.is_flagship && (
        <span style={{ position: 'absolute', top: '-10px', left: '20px', ...sc, fontSize: '13px', letterSpacing: '0.16em', background: '#C8922A', color: '#FFFFFF', padding: '3px 12px', borderRadius: '40px' }}>Flagship</span>
      )}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
            <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', color: gold, background: 'rgba(200,146,42,0.08)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '4px', padding: '3px 10px' }}>{typeLabel}</span>
            <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.10em', color: 'rgba(15,21,35,0.55)', background: 'rgba(15,21,35,0.55)', border: '1px solid rgba(15,21,35,0.55)', borderRadius: '4px', padding: '3px 10px' }}>{modeLabel}</span>
            <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.10em', color: 'rgba(15,21,35,0.55)', background: 'rgba(15,21,35,0.55)', border: '1px solid rgba(15,21,35,0.55)', borderRadius: '4px', padding: '3px 10px' }}>{accessLabel}</span>
          </div>
          ...
          {offering.domain_ids?.length > 0 && (
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '4px' }}>
              {offering.domain_ids.map(d => (
                <span key={d} style={{ ...sc, fontSize: '13px', letterSpacing: '0.10em', color: 'rgba(15,21,35,0.55)', background: 'rgba(15,21,35,0.55)', borderRadius: '4px', padding: '2px 8px' }}>{DOMAIN_LABEL[d] || d}</span>
              ))}
            </div>
          )}
          {offering.url && (
            <a href={offering.url} target="_blank" rel="noopener noreferrer" style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', color: gold, textDecoration: 'none', marginTop: '6px', display: 'inline-block' }}>Visit →</a>
          )}
        </div>
...
function ContributionsTab({ actorId, actorName, toast }) {
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
                    <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', color: gold, background: 'rgba(200,146,42,0.08)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '4px', padding: '3px 10px' }}>
                      {CONTRIBUTION_TYPE_LABEL[c.contribution_type] || c.contribution_type}
                    </span>
                    {c.amount && <span style={{ ...sc, fontSize: '13px', color: 'rgba(15,21,35,0.55)' }}>{c.contribution_type === 'capital' ? `${c.currency} ${c.amount}` : `${c.amount} hrs`}</span>}
                  </div>
      ...
      {confirmed.length > 0 && (
        <>
          <p style={{ ...sc, fontSize: '13px', letterSpacing: '0.16em', color: 'rgba(15,21,35,0.55)', marginTop: '28px', marginBottom: '12px' }}>
            ...
            <SectionCard key={c.id} style={{ marginBottom: '12px' }}>
              <div style={{ marginBottom: c.outcome_reported ? 0 : '16px' }}>
                <div style={{ display: 'flex', gap: '8px', marginBottom: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', color: '#2A6B3A', background: 'rgba(42,107,58,0.08)', border: '1px solid rgba(42,107,58,0.25)', borderRadius: '4px', padding: '3px 10px' }}>
                    {CONTRIBUTION_TYPE_LABEL[c.contribution_type] || c.contribution_type}
                  </span>
                  {c.amount && <span style={{ ...sc, fontSize: '13px', color: 'rgba(15,21,35,0.55)' }}>{c.contribution_type === 'capital' ? `${c.currency} ${c.amount}` : `${c.amount} hrs`}</span>}
...
function ContributorMatchCard({ match }) {
  ...
  return (
    <div style={{ background: match.adjacent ? '#FFFFFF' : 'rgba(200,146,42,0.04)', border: match.adjacent ? '1.5px solid rgba(200,146,42,0.18)' : '1.5px solid rgba(200,146,42,0.55)', borderRadius: '12px', padding: '18px 20px', marginBottom: '10px' }}>
      {match.adjacent && <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', color: 'rgba(15,21,35,0.55)', display: 'block', marginBottom: '6px' }}>Adjacent match</span>}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', gap: '6px', marginBottom: '6px', flexWrap: 'wrap' }}>
            <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', color: gold, background: 'rgba(200,146,42,0.08)', border: '1px solid rgba(200,146,42,0.22)', borderRadius: '4px', padding: '2px 8px' }}>{typeLabel}</span>
            <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.10em', color: 'rgba(15,21,35,0.55)', background: 'rgba(15,21,35,0.55)', borderRadius: '4px', padding: '2px 8px' }}>{modeLabel}</span>
            <span style={{ ...sc, fontSize: '13px', color: 'rgba(15,21,35,0.55)' }}>{returnLabel}</span>
          ...
          {match.display_name && (
            <p style={{ ...body, fontSize: '16px', fontWeight: 400, color: dark, marginBottom: '2px' }}>
              {match.display_name}
              {match.archetype && <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.10em', color: 'rgba(168,114,26,0.70)', marginLeft: '10px' }}>{match.archetype}</span>}
            </p>
          )}
          ...
          {match.best_need && <p style={{ ...sc, fontSize: '13px', letterSpacing: '0.10em', color: 'rgba(15,21,35,0.55)', marginTop: '6px' }}>Matches your need: {match.best_need.title}</p>}
          {match.availability && <p style={{ ...sc, fontSize: '13px', color: 'rgba(15,21,35,0.55)', marginTop: '4px' }}>{match.availability}</p>}
        </div>
        <a href={`/profile/${match.user_id}`} style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', padding: '8px 16px', borderRadius: '40px', flexShrink: 0, border: '1.5px solid rgba(200,146,42,0.60)', background: 'rgba(200,146,42,0.04)', color: gold, textDecoration: 'none', whiteSpace: 'nowrap' }}>
          View profile →
        </a>
      </div>
    </div>
  ...
  return (
    <div style={{ background: parch, minHeight: '100vh' }}>
      ...
      <div className="beta-manage-main" style={{ maxWidth: '820px', margin: '0 auto', padding: '80px 40px 120px' }}>
        ...
        <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.20em', color: gold, display: 'block', marginBottom: '10px' }}>
          Managing
        </span>
        ...
        {actor.needs_visible === false && (
          <div style={{ background: 'rgba(138,48,48,0.04)', border: '1.5px solid rgba(138,48,48,0.35)', borderRadius: '12px', padding: '18px 22px', marginBottom: '28px' }}>
            <p style={{ ...sc, fontSize: '13px', letterSpacing: '0.18em', color: '#8A3030', marginBottom: '6px' }}>
              Needs hidden
            </p>
