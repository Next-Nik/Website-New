    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '12px 20px 6px' }}>
          <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.20em', color: '#262420', textTransform: 'uppercase' }}>NextUs Self</span>
        </div>
        {lifeOSLinks.map(l => (
        ...
        {/* NextUs */}
        <div style={{ padding: '12px 20px 6px', borderTop: '1px solid rgba(76,107,69,0.08)' }}>
          <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.20em', color: '#262420', textTransform: 'uppercase' }}>NextUs</span>
        </div>
        {nextusLinks.map(l => (
          <Link key={l.path} to={l.path} onClick={onClose} style={{
            display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '8px',
        ...
        {/* Site links */}
        <div style={{ borderTop: '1px solid rgba(76,107,69,0.15)' }}>
          {siteLinks.map((l, i) => (
            <Link key={l.path} to={l.path} onClick={onClose} style={{
              display: 'block', padding: '14px 20px',
              ...sc, fontSize: '15px', letterSpacing: '0.10em',
              color: 'rgba(15,21,35,0.72)', textDecoration: 'none',
              borderBottom: i < siteLinks.length - 1 ? '1px solid rgba(76,107,69,0.08)' : 'none',
            }}>{l.label}</Link>
          ))}
        </div>
      </div>
        ...
        {tabs.map(tab => {
          const inner = (
            <div style={{
              ...
              <span style={{
                ...sc, fontSize: '15px', letterSpacing: '0.10em',
                color: tab.active ? '#262420' : 'rgba(15,21,35,0.55)',
                lineHeight: 1,
              }}>{tab.label}</span>
            </div>
          )
