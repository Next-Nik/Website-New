...
module.exports = async (req, res) => {
  ...
  // The living surface's fuel: recent real events in the founding tree, plus
  // the field (challenges with tallies, the organisations present) and today's
  // spark count. Public, no auth. Identity policy: people appear as first name
  // plus an initial; organisations appear as themselves.
  if (action === 'constellation_activity') {
    // Logic lives in api/_beaconStrip.js, shared with /api/beacon action 'strip'.
    const out = await constellationActivity(supabase, body.limit)
    return res.json(out)
  }
  ...
  if (action === 'my_participations') {
    if (!userId) return res.status(401).json({ error: 'Auth required' })
    // Logic lives in api/_beaconStrip.js, shared with /api/beacon action 'strip'.
    const participations = await myParticipations(supabase, userId)
