  ...
  return (
    <div style={{ minHeight: '100dvh', background: at.ground, color: at.text }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '44px 24px 90px' }}>
        {loading && <p style={{ ...body, color: at.ghost }}>Reading the record…</p>}

        {!loading && (!tally || !tally.rooted) && (
          <div>
            <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.22em', textTransform: 'uppercase', color: at.brass }}>The record</div>
            <h1 style={{ ...serif, fontWeight: 500, fontSize: '40px', lineHeight: 1.1, margin: '12px 0 8px' }}>The constellation is just beginning.</h1>
            <p style={{ ...body, fontSize: '18px', color: at.ghost, maxWidth: '46ch' }}>
        ...
        {!loading && tally && tally.rooted && (
          <div>
            <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.18em', textTransform: 'uppercase', color: at.brass }}>
            ...
            <p style={{ ...body, fontSize: '18px', color: at.ghost, maxWidth: '48ch' }}>
            ...
            <div style={{ margin: '34px 0 0' }}>
              <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.18em', textTransform: 'uppercase', color: at.brass, marginBottom: '8px' }}>
                Every challenge, what got done
              </div>
              {rows.length === 0 && (
                <p style={{ ...body, color: at.ghost }}>The first actions are still landing.</p>
              )}
              {rows.map((c) => (
                <div key={c.call_id} style={{ padding: '16px 0', borderBottom: `1px solid ${tokens.goldFaint}` }}>
                  <div style={{ ...serif, fontSize: '24px', lineHeight: 1.2, color: at.text }}>{c.the_move || c.title}</div>
                  <div style={{ ...body, fontSize: '15px', color: at.ghost, marginTop: '3px' }}>
                    <b style={{ color: AMBER_DEEP, fontWeight: 500 }}>{fmt(c.done)}</b> {c.done === 1 ? 'time' : 'times'} done, by <b style={{ color: AMBER_DEEP, fontWeight: 500 }}>{fmt(c.people)}</b> {c.people === 1 ? 'person' : 'people'}{c.actor_name ? <> · <span style={{ color: at.brass }}>{c.actor_name}</span></> : null}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '24px' }}>
              <button onClick={share} style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#fff', background: at.verdigris, border: 'none', borderRadius: '26px', padding: '13px 26px', cursor: 'pointer' }}>
                Share the record
              </button>
            </div>
            <p style={{ ...body, fontSize: '13px', color: at.ghost, textAlign: 'center', marginTop: '14px' }}>
