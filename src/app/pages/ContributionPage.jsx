      ...
      {DOMAINS.map(d => (
        <button key={d.id} type="button" onClick={() => onChange(d.id)}
          style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', padding: '6px 16px', borderRadius: '20px', cursor: 'pointer', transition: 'all 0.2s',
            border: `1px solid ${value === d.id ? 'rgba(76,107,69,0.78)' : 'rgba(76,107,69,0.3)'}`,
            background: value === d.id ? 'rgba(76,107,69,0.08)' : 'transparent',
  ...
  return (
    <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '60px 0' }}>
      ...
      <a href="/tools/target-sprint"
        style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', ...gold, textDecoration: 'none', border: '1px solid rgba(76,107,69,0.5)', borderRadius: '30px', padding: '8px 20px', display: 'inline-block' }}>
  ...
  return (
    <div style={{ background: tokens.bg, minHeight: '100dvh' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: 'clamp(64px,8vw,96px) clamp(20px,5vw,40px) 100px' }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: '0', marginBottom: '24px', borderBottom: '1px solid rgba(76,107,69,0.18)' }}>
          {[
            { id: 'asks',     l: 'Open asks' },
        ...
        {/* Post an ask CTA */}
        {user && tab === 'asks' && (
          <div style={{ marginTop: '40px', paddingTop: '28px', borderTop: '1px solid rgba(76,107,69,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
