...
export default function ConstellationLedger() {
  ...
  if (authLoading || user === undefined) {
    return <div style={{ minHeight: '100dvh', background: tokens.bg }}><Nav /></div>
  }
    ...
    return (
      <div style={{ minHeight: '100dvh', background: tokens.bg, color: tokens.dark }}>
        ...
        <div style={{ maxWidth: '560px', margin: '0 auto', padding: '60px 24px', textAlign: 'center' }}>
          <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.22em', textTransform: 'uppercase', color: tokens.gold }}>Stewards only</div>
          <h1 style={{ ...serif, fontWeight: 500, fontSize: '34px', margin: '10px 0 8px' }}>This is the inside view.</h1>
          <p style={{ ...body, color: tokens.ghost }}>The ledger is for the constellation&rsquo;s stewards. What everyone can see lives in the record.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100dvh', background: tokens.bg, color: tokens.dark }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '44px 24px 90px' }}>
        <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.22em', textTransform: 'uppercase', color: tokens.gold }}>Steward view</div>
        <h1 style={{ ...serif, fontWeight: 500, fontSize: '44px', lineHeight: 1.04, margin: '12px 0 6px' }}>The constellation, in full.</h1>
        <p style={{ ...body, fontSize: '18px', color: tokens.ghost, maxWidth: '54ch' }}>
          Every challenge, what it is, and what it gathered. Spark counts per challenge live here, not on any public page.
        </p>

        {loading && <p style={{ ...body, color: tokens.ghost, marginTop: '24px' }}>Reading the ledger…</p>}
          ...
          <>
            <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', margin: '28px 0 8px' }}>
              {[
                ...
                <div key={i} style={{ flex: n.hero ? '1.5 1 0' : '1 1 0', minWidth: '130px', border: `1px solid ${n.hero ? tokens.goldFaint : 'rgba(15,21,35,0.12)'}`, borderRadius: '14px', padding: '16px 18px', background: tokens.bgCard }}>
                  <div style={{ ...serif, fontWeight: 500, fontSize: n.hero ? '50px' : '38px', lineHeight: 1, color: n.hero ? AMBER_DEEP : tokens.dark, fontVariantNumeric: 'tabular-nums' }}>{n.v}</div>
                  <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', textTransform: 'uppercase', color: tokens.ghost, marginTop: '5px' }}>{n.k}</div>
                </div>
              ))}
            </div>
              ...
              {rows.map((c) => (
                <div key={c.call_id} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', alignItems: 'center', padding: '16px 0', borderBottom: `1px solid ${tokens.goldFaint}` }}>
                  <div style={{ minWidth: 0 }}>
                    {c.actor_name && <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.1em', textTransform: 'uppercase', color: tokens.gold }}>{c.actor_name}</div>}
                    ...
                    <div style={{ height: '3px', borderRadius: '3px', background: tokens.goldGlow, marginTop: '8px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${Math.round((c.sparks / maxSparks) * 100)}%`, background: tokens.goldChrome, borderRadius: '3px' }} />
                    </div>
                  </div>
                  <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <div style={{ ...serif, fontSize: '30px', color: AMBER_DEEP, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{fmt(c.sparks)}</div>
                    <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.12em', textTransform: 'uppercase', color: tokens.ghost }}>sparks</div>
                  </div>
                </div>
              ))}
              {rows.length === 0 && <p style={{ ...body, color: tokens.ghost, marginTop: '16px' }}>No challenges have gathered sparks yet.</p>}
            </div>
          </>
        )}
      </div>
