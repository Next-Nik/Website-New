...
function ChallengeCard({ c, colour }) {
  return (
    <Link to={c.slug ? `/stretch/c/${c.slug}` : '#'}
      ...
      {c.the_move && <p style={{ ...body, fontSize: '15px', color: 'rgba(234,241,237,0.72)', lineHeight: 1.55, margin: '6px 0 0' }}>{c.the_move}</p>}
      <div style={{ display: 'flex', gap: '14px', marginTop: '12px', ...sc, fontSize: '13px', letterSpacing: '0.08em', color: 'rgba(234,241,237,0.58)' }}>
  ...
  return (
    <Link to={a.slug ? `/org/${a.slug}` : '#'}
      ...
      <span style={{ minWidth: 0 }}>
        ...
        {a.tagline && <span style={{ display: 'block', ...body, fontSize: '13px', color: 'rgba(234,241,237,0.58)', lineHeight: 1.4, marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.tagline}</span>}
      </span>
    </Link>
  )
}
...
function SectionLabel({ children }) {
  return <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.18em', color: 'rgba(234,241,237,0.58)', textTransform: 'uppercase', margin: '36px 0 14px' }}>{children}</div>
}
  ...
  if (notFound || !data) return (
    <div style={{ background: at.ground, minHeight: '100dvh' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '120px 24px', textAlign: 'center' }}>
        ...
        <p style={{ ...body, fontSize: '1.0625rem', color: 'rgba(234,241,237,0.78)', lineHeight: 1.7 }}>
  ...
  return (
    <div style={{ background: at.ground, minHeight: '100dvh' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: 'clamp(56px,7vw,88px) clamp(20px,5vw,40px) 100px' }}>
        ...
        <p style={{ ...body, fontSize: '1.0625rem', color: 'rgba(234,241,237,0.72)', lineHeight: 1.7, margin: '0 0 20px' }}>
        ...
        {calls.length === 0 && actors.length === 0 && (
          <p style={{ ...body, fontSize: '1.0625rem', color: 'rgba(234,241,237,0.58)', lineHeight: 1.7, marginTop: '36px' }}>
