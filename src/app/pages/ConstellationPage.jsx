...
function ShareButton({ title }) {
  ...
  return (
    <button type="button" onClick={share}
      style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', textTransform: 'uppercase', color: at.brass, background: 'rgba(169,116,63,0.08)', border: '1.5px solid rgba(169,116,63,0.78)', borderRadius: '30px', padding: '10px 22px', cursor: 'pointer' }}>
...
function ChallengeCard({ c, colour }) {
  return (
    <Link to={c.slug ? `/stretch/c/${c.slug}` : '#'}
      ...
      {c.the_move && <p style={{ ...body, fontSize: '15px', color: 'rgba(38,36,32,0.72)', lineHeight: 1.55, margin: '6px 0 0' }}>{c.the_move}</p>}
      <div style={{ display: 'flex', gap: '14px', marginTop: '12px', ...sc, fontSize: '13px', letterSpacing: '0.08em', color: 'rgba(38,36,32,0.58)' }}>
...
function ActorCard({ a, colour }) {
  ...
  return (
    <Link to={a.slug ? `/org/${a.slug}` : '#'}
      style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '13px', background: at.object, border: '1px solid rgba(169,116,63,0.18)', borderRadius: '12px', padding: '14px 16px' }}>
      {a.image_url
      ...
      <span style={{ minWidth: 0 }}>
        ...
        {a.tagline && <span style={{ display: 'block', ...body, fontSize: '13px', color: 'rgba(38,36,32,0.58)', lineHeight: 1.4, marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.tagline}</span>}
      </span>
    </Link>
  )
}
...
function SectionLabel({ children }) {
  return <div style={{ ...sc, fontSize: '13px', letterSpacing: '0.18em', color: 'rgba(38,36,32,0.58)', textTransform: 'uppercase', margin: '36px 0 14px' }}>{children}</div>
}
  ...
  if (notFound || !data) return (
    <div style={{ background: at.ground, minHeight: '100dvh' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '120px 24px', textAlign: 'center' }}>
        ...
        <p style={{ ...body, fontSize: '1.0625rem', color: 'rgba(38,36,32,0.78)', lineHeight: 1.7 }}>
  ...
  return (
    <div style={{ background: at.ground, minHeight: '100dvh' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: 'clamp(56px,7vw,88px) clamp(20px,5vw,40px) 100px' }}>
        ...
        <p style={{ ...body, fontSize: '1.0625rem', color: 'rgba(38,36,32,0.72)', lineHeight: 1.7, margin: '0 0 20px' }}>
        ...
        {calls.length === 0 && actors.length === 0 && (
          <p style={{ ...body, fontSize: '1.0625rem', color: 'rgba(38,36,32,0.58)', lineHeight: 1.7, marginTop: '36px' }}>
