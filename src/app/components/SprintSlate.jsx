...
function SlotEmpty({ slot }) {
  return (
    <div style={{
      padding: '20px 18px',
      border: '1px dashed rgba(76,107,69,0.20)',
      borderRadius: '14px',
      minHeight: '120px',
      display: 'flex',
  ...
  return (
    <select
      ...
      style={{
        ...
        fontSize: '13px',
        letterSpacing: '0.1em',
        color: value === 'public' ? '#262420' : 'rgba(15,21,35,0.55)',
...
function SlotFilled({ slot, session, visibilityValue, onToggleVisibility }) {
  ...
  return (
    <div style={{
      ...
      {/* Goal text — italic permitted: this is the user's own sprint goal */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        {status && (
          <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.1em', color: status.color }}>
            {status.text}
          </span>
        )}
        <a
          href="/tools/target-sprint"
          style={{ ...sc, fontSize: '13px', letterSpacing: '0.1em', color: '#262420', textDecoration: 'none', marginLeft: 'auto' }}
        >
          Open sprint
        </a>
      </div>
