// NextSteps — Stage 5 (Loop)
// src/tools/nextsteps/stages/TrackLoop.jsx
//
  ...
  return (
    <div className="ns-loop">
      ...
      <ul className="ns-track-list">
        {sorted.map((track) => (
          ...
          >
            ...
            {/* Where they are in the route. Structural position, taken from
                which exit condition has not yet been met. Never a percentage
                and never a date, because a phase has no duration to be
                measured against. */}
            {track.current_phase_name && (
              <div className="ns-track-phase">
                <span className="ns-track-phase-pos">
                  Phase {track.current_phase_position} of {track.total_phases}
                </span>
                <span className="ns-track-phase-name">{track.current_phase_name}</span>
              </div>
            )}

            {track.route_state === 'drafted' && (
              <div className="ns-track-phase ns-track-phase-draft">
                <span className="ns-track-phase-pos">Route sketched</span>
                <span className="ns-track-phase-name">Waiting for you to make it yours</span>
              </div>
            )}
            ...
            <div className="ns-track-meta">
              ...
              {typeof track.total_phases === 'number' && track.total_phases > 0 && (
                <span className="ns-track-steps">
                  {track.cleared_phases || 0} of {track.total_phases} phases cleared
                </span>
              )}
      ...
      <style>{`
        ...
        .ns-track-phase-draft .ns-track-phase-pos { color: #a9743f; }
        .ns-track-phase-draft .ns-track-phase-name { color: rgba(38,36,32,0.68); }
        .ns-track-card {
          padding: 20px 22px;
          background: #FFFFFF;
