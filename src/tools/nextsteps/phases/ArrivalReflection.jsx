...
export function ArrivalReflection({ user, onReflectionLanded }) {
  ...
  useEffect(() => {
    if (!prefilledQ || autoSubmittedRef.current || initialDraft) return
    autoSubmittedRef.current = true
    const userMsg = { role: 'user', content: prefilledQ }
    const next = [{ role: 'assistant', content: OPENING }, userMsg]
    originalConcernRef.current = prefilledQ
    setTimeout(() => {
      setMessages(next)
      setThinking(true)
      fetch('/tools/nextsteps/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          messages: next.map(m => ({ role: m.role, content: m.content })),
        ...
        .then(data => {
          if (data.reflection) {
            onReflectionLanded(data.reflection, prefilledQ, [...next])
          } else {
            setMessages(prev => [...prev, { role: 'assistant', content: data.message }])
          }
        })
        .catch(() => setMessages(prev => [...prev, { role: 'assistant', content: 'Something went wrong — try again in a moment.' }]))
        .finally(() => setThinking(false))
    }, 300)
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  const isFirstLoad = messages.length === 1 && !thinking

  return (
    <div className={`ns-chat${isFirstLoad ? ' ns-chat--fresh' : ''}`}>
        ...
        {isFirstLoad && !landed && (
          <div className="ns-input-inline">
            <textarea
              ...
              autoFocus
            />
            <button type="button" className="ns-send" onClick={send} disabled={!input.trim() || thinking}>Send</button>
          </div>
        )}
      </div>
      {!isFirstLoad && !landed && (
      ...
      <style>{`
        .ns-chat { display:flex; flex-direction:column; height:calc(100dvh - 56px); padding:0; position:relative; }
        ...
        .ns-bubble.assistant.ns-opener { background:transparent; border:none; padding:0 0 20px; font-family:'Fraunces',Georgia,serif; font-size:clamp(1.35rem,2.2vw,1.65rem); font-weight:400; line-height:1.55; color:rgba(15,21,35,0.88); max-width:100%; border-bottom:1px solid rgba(38,36,32,0.18); }
        ...
        .ns-input-row { flex-shrink:0; display:flex; gap:10px; align-items:flex-end; background:#FFFFFF; border-top:1px solid rgba(38,36,32,0.18); padding:14px 24px; max-width:680px; width:100%; margin:0 auto; box-sizing:border-box; }
        .ns-input { flex:1; border:1px solid rgba(38,36,32,0.25); border-radius:3px; outline:none; resize:none; font-family:'Newsreader',Georgia,serif; font-size:1rem; line-height:1.55; color:#0F1523; background:#FFFFFF; padding:10px 12px; min-height:44px; }
        ...
        .ns-send { background:#4c6b45; color:#FFFFFF; border:none; border-radius:3px; padding:10px 20px; font-family:'IBM Plex Mono',Georgia,serif; font-size:0.8rem; letter-spacing:0.16em; text-transform:uppercase; cursor:pointer; transition:background 0.15s; flex-shrink:0; height:44px; }
        .ns-send:hover:not(:disabled) { background:#262420; }
        .ns-send:disabled { background:rgba(15,21,35,0.15); cursor:not-allowed; }
        .ns-typing { align-self:flex-start; display:inline-flex; gap:5px; padding:16px 20px; background:#FFFFFF; border:1px solid rgba(38,36,32,0.16); border-radius:3px; box-shadow:0 2px 8px rgba(15,21,35,0.06); }
        .ns-typing span { width:6px; height:6px; border-radius:50%; background:rgba(38,36,32,0.50); animation:ns-bounce 1.2s infinite; }
        ...
        @media (max-width:640px) {
          .ns-msgs { padding:28px 18px 16px; }
          .ns-chat--fresh .ns-msgs { padding:0 18px; }
