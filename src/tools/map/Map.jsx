...
import MissionWheel from '../../app/components/mission-control/MissionWheel'
  ...
  return (
    <div style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', marginBottom: '10px' }}>
      <div style={{
        ...
        background: isUser ? 'rgba(200,146,42,0.08)' : '#FFFFFF',
  ...
  return (
    <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
      <textarea
        ...
        rows={2}
        disabled={disabled}
        style={{ flex: 1, padding: '10px 14px', fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '1.125rem', color: 'rgba(15,21,35,0.78)', background: 'rgba(200,146,42,0.05)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '10px', outline: 'none', resize: 'none', lineHeight: 1.55 }}
      />
      ...
      {step === 'score' && !isConnection && (
        <div>
          ...
          <textarea
            ...
            rows={4}
            style={{ width: '100%', marginTop: '16px', padding: '12px 14px', fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '1.25rem', color: 'rgba(15,21,35,0.78)', background: 'rgba(200,146,42,0.05)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '8px', outline: 'none', resize: 'vertical', lineHeight: 1.65 }}
          />
...
export function ResultsCard({ mapData, domainData, currentScores, horizonScores }) {
  ...
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid rgba(200,146,42,0.25)', borderLeft: '3px solid rgba(200,146,42,0.55)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 20px rgba(200,146,42,0.08)', animation: 'fadeUp 0.5s ease-out' }}>
      ...
      {/* Hero */}
      <div style={{ padding: '28px 28px 22px', borderBottom: '1px solid rgba(200,146,42,0.1)', background: 'rgba(200,146,42,0.05)' }}>
      ...
      {(() => {
        ...
        return (
          <div style={{ padding: '16px 28px', borderBottom: '1px solid rgba(200,146,42,0.07)', background: 'rgba(200,146,42,0.05)' }}>
...
export function MapPage() {
  ...
  // ── SelfWheel props derived from existing domainData / scores ─────────────
  const SELF_LABELS = useMemo(() => DOMAINS.map(d => d.label), [])
  const SELF_KEYS   = useMemo(() => DOMAINS.map(d => d.id),    [])
  const wheelCurrent  = useMemo(() => {
    const out = {}
    DOMAINS.forEach(d => {
      if (domainData[d.id]?.currentScore !== undefined)
        out[d.id] = domainData[d.id].currentScore
    })
    return out
  }, [domainData])
  const wheelHorizons = useMemo(() => {
    const out = {}
    DOMAINS.forEach(d => {
      if (domainData[d.id]?.horizonScore !== undefined)
        out[d.id] = domainData[d.id].horizonScore
    })
    return out
  }, [domainData])
  const wheelActiveKey = activeIndex !== null ? DOMAINS[activeIndex]?.id ?? null : null
  const wheelIsEmpty   = Object.keys(wheelCurrent).length === 0
  ...
  return (
    <div className="page-shell">
      ...
      <div className="tool-wrap">
        ...
        {/* Header — static when not in mapping phase; wheel band takes over during mapping */}
        {phase !== 'mapping' && (
        ...
        {/* Mapping phase */}
        {phase === 'mapping' && (() => {
          // Active domain colour for accent chrome
          const dc = activeDomain ? selfColor(activeDomain.id) : null

          // ── Wheel band — shared across mobile and desktop ───────────────
          const WheelBand = (
            <div style={{
              position: 'relative',
              background: '#FAFAF7',
              ...
              borderBottom: '0.5px solid rgba(200,146,42,0.18)',
              marginLeft: '-1.5rem',
              marginRight: '-1.5rem',
              marginTop: '-2.5rem',
              marginBottom: '28px',
            }}>
              {/* Active domain name — top of band, in domain colour */}
              <div style={{
                position: 'relative',
                zIndex: 2,
                textAlign: 'center',
                ...
                fontWeight: 700,
                letterSpacing: '0.2em',
                color: dc ? dc.light : '#A8721A',
                lineHeight: 1,
                transition: 'color 0.3s ease',
                userSelect: 'none',
              }}>
                {activeDomain ? activeDomain.label : 'The Map'}
              </div>

              {/* Wheel — centred, clipped at bottom */}
              <div style={{
                position: 'absolute',
                top: isMobile ? '32px' : '44px',
                left: '50%',
                ...
                <MissionWheel
                  ...
                  labels={SELF_LABELS}
                  keys={SELF_KEYS}
                  horizons={wheelHorizons}
                  current={wheelCurrent}
                  activeKey={wheelActiveKey}
                  isEmpty={wheelIsEmpty}
                  onSelect={i => setActiveIndex(i)}
                  onCentreClick={() => setThreadPanelOpen(p => !p)}
                />
              </div>

              {/* Fade — wheel dissolves into card */}
              <div style={{
                position: 'absolute',
                bottom: 0, left: 0, right: 0,
                height: '80px',
                background: 'linear-gradient(to top, #FAFAF7 40%, transparent 100%)',
                pointerEvents: 'none',
                zIndex: 1,
              }} />
              ...
              <div style={{
                ...
                <div style={{
                  height: '100%',
                  background: '#C8922A',
                  width: `${(Object.values(domainData).reduce((s, d) => s + getDomainStage(d), 0) / (DOMAINS.length * 3)) * 100}%`,
          ...
          const PrevNext = (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '6px', opacity: 0.45, transition: 'opacity 0.2s' }}
          ...
          // ── Domain card shell — same content as before, new chrome ──────
          const domainCardStyle = {
            background: '#FAFAF7',
            border: `1.5px solid ${dc ? dc.border : 'rgba(200,146,42,0.25)'}`,
            borderTop: `3px solid ${dc ? dc.base : 'rgba(200,146,42,0.55)'}`,
            borderRadius: '12px',
              ...
              <div style={domainCardStyle}>
                {activeDomain ? (
                  ...
                  <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '1.25rem', fontStyle: 'italic', color: 'rgba(15,21,35,0.78)', textAlign: 'center', padding: '20px 0' }}>
                    Select a domain from the wheel above to begin.
                  </p>
                )}
                {allComplete && (
