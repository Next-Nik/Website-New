...
function HourglassPicker({ onScore, horizonMode = false, currentScore }) {
  ...
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid rgba(200,146,42,0.2)', borderRadius: '12px', padding: '16px 20px', marginTop: '12px' }}>
      {horizonMode && (
        <div style={{ fontFamily: 'var(--font-sc)', fontSize: '0.5625rem', letterSpacing: '0.16em', color: 'var(--gold-dk)', textTransform: 'uppercase', marginBottom: '12px', paddingBottom: '10px', borderBottom: '1px solid rgba(200,146,42,0.12)' }}>
        ...
        {points.map(n => {
          ...
          return (
            <div key={n} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '28px', textAlign: 'right', fontFamily: 'var(--font-sc)', fontSize: '0.625rem', letterSpacing: '0.04em', color: isLine ? 'var(--gold-dk)' : isCur ? c : 'rgba(15,21,35,0.45)', fontWeight: (isLine || isCur) ? 600 : 400, flexShrink: 0 }}>{n}</div>
              ...
              <div style={{ width: '130px', flexShrink: 0, fontFamily: 'var(--font-body)', fontSize: '0.625rem', color: isLine ? 'var(--gold-dk)' : isCur ? c : 'rgba(15,21,35,0.35)', fontWeight: isCur ? 600 : 400, letterSpacing: '0.03em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {LABEL_MAP[n]}
              </div>
            </div>
          )
            ...
            {stage === 3 && score !== undefined ? (
              ...
              <>
                ...
                <text x={p.x} y={stage > 0 ? p.y + 7 : p.y} textAnchor="middle" dominantBaseline="middle"
                  fill={isActive ? 'var(--gold-dk)' : stage > 0 ? 'rgba(200,146,42,0.7)' : 'rgba(15,21,35,0.55)'}
...
export function DomainThreadPanel({ domainData, activeIndex, onSelect }) {
  ...
  return (
    <>
      ...
      >
        <span style={{ fontFamily: 'var(--font-sc)', fontSize: '9px', letterSpacing: '0.14em', color: 'var(--gold-dk)', writingMode: 'vertical-rl', textOrientation: 'mixed', transform: 'rotate(180deg)' }}>
          DOMAINS
        </span>
        <span style={{ color: 'var(--gold-dk)', fontSize: '12px', marginTop: '4px' }}>
          {open ? '‹' : '›'}
        </span>
      </button>
      ...
      <div style={{
        ...
        <div style={{ padding: '0 20px 20px' }}>
          <div style={{ fontFamily: 'var(--font-sc)', fontSize: '10px', letterSpacing: '0.2em', color: 'var(--gold-dk)', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid rgba(200,146,42,0.15)' }}>
            DOMAIN STATUS
          </div>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                  <span style={{ fontFamily: 'var(--font-sc)', fontSize: '12px', color: stage === 3 ? 'rgba(200,146,42,0.9)' : stage > 0 ? 'rgba(200,146,42,0.6)' : 'rgba(15,21,35,0.3)' }}>
                    {STAGE_ICONS[stage]}
                  </span>
                  <span style={{ fontFamily: 'var(--font-sc)', fontSize: '11px', letterSpacing: '0.08em', color: isActive ? 'var(--gold-dk)' : 'rgba(15,21,35,0.72)' }}>
                    {domain.label}
                  </span>
                  {score !== undefined && (
                    <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-sc)', fontSize: '12px', fontWeight: 600, color: getScoreColor(score) }}>
                      {score}
                    </span>
                  )}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: '11px', fontStyle: 'italic', color: 'rgba(15,21,35,0.4)' }}>
                    {STAGE_LABELS[stage]}
                  </span>
                  {horizon !== undefined && (
                    <span style={{ fontFamily: 'var(--font-sc)', fontSize: '10px', color: 'rgba(200,146,42,0.6)' }}>
                      → {horizon}
                    </span>
                  )}
                </div>
          ...
          <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(200,146,42,0.12)' }}>
            {[
              ...
              <div key={item.icon} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                <span style={{ fontFamily: 'var(--font-sc)', fontSize: '11px', color: 'rgba(200,146,42,0.55)', width: '14px' }}>{item.icon}</span>
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '11px', color: 'rgba(15,21,35,0.45)' }}>{item.label}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(15,21,35,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: '#FAFAF7', border: '1.5px solid rgba(200,146,42,0.78)', borderRadius: '14px', padding: '32px 28px', maxWidth: '400px', width: '100%' }}>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', fontWeight: 300, color: 'var(--text)', lineHeight: 1.75, marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <button onClick={onCancel} style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontStyle: 'italic', color: 'rgba(15,21,35,0.45)', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>Cancel</button>
        </div>
      </div>
    </div>
  )
...
const btnStyle = {
  fontFamily: 'var(--font-sc)', fontSize: '0.875rem', letterSpacing: '0.12em',
    ...
    return (
      <div style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', marginBottom: '10px' }}>
        <div style={{
          ...
          fontFamily: 'var(--font-body)', fontSize: '0.9375rem', fontWeight: 300,
          color: isUser ? 'rgba(15,21,35,0.72)' : 'var(--text-meta)',
          lineHeight: 1.72,
    ...
    return (
      <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
        <textarea
          ...
          rows={2}
          disabled={disabled}
          style={{ flex: 1, padding: '10px 14px', fontFamily: 'var(--font-body)', fontSize: '0.9375rem', color: 'var(--text-meta)', background: 'rgba(200,146,42,0.02)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '10px', outline: 'none', resize: 'none', lineHeight: 1.55 }}
        />
    ...
    <div style={{ background: '#FFFFFF', border: '1px solid rgba(200,146,42,0.2)', borderLeft: '3px solid rgba(200,146,42,0.55)', borderRadius: '12px', padding: '24px 24px 20px', animation: 'fadeUp 0.3s ease-out' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
          ...
          {stage === 3 && (
            <span style={{ fontFamily: 'var(--font-sc)', fontSize: '0.5625rem', letterSpacing: '0.14em', color: 'rgba(200,146,42,0.7)' }}>● Complete</span>
          )}
        </div>
        ...
        {flagReview && step !== 'done' && (
          <div style={{ marginTop: '8px', padding: '8px 12px', background: 'rgba(200,146,42,0.06)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '8px', fontFamily: 'var(--font-body)', fontSize: '0.8125rem', fontStyle: 'italic', color: 'rgba(200,146,42,0.8)' }}>
        ...
        {['avatar', 'score', 'horizon'].map((s, i) => {
          ...
          return (
            <button key={s} onClick={() => reachable && setStep(s)}
              style={{
                fontFamily: 'var(--font-sc)', fontSize: '0.625rem', letterSpacing: '0.12em',
                padding: '8px 14px', background: 'none', border: 'none',
                borderBottom: active ? '2px solid var(--gold-dk)' : '2px solid transparent',
                marginBottom: '-1px', cursor: reachable ? 'pointer' : 'default',
                color: active ? 'var(--gold-dk)' : reachable ? 'rgba(15,21,35,0.55)' : 'rgba(15,21,35,0.25)',
              }}>
              {labels[i]}
            </button>
          )
      ...
      {(step === 'avatar') && (
        <div>
          {!avatarLocked || editingAvatar ? (
            <>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9375rem', color: 'var(--text-meta)', lineHeight: 1.75, marginBottom: '16px' }}>
              ...
              {avatarMessages.length === 0 && (
                <textarea
                  ...
                  rows={4}
                  style={{ width: '100%', padding: '12px 14px', fontFamily: 'var(--font-body)', fontSize: '1rem', color: 'var(--text-meta)', background: 'rgba(200,146,42,0.02)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '8px', outline: 'none', resize: 'vertical', lineHeight: 1.65, marginBottom: '10px' }}
                />
              )}
            ...
            <div>
              <div style={{ padding: '14px 16px', background: 'rgba(200,146,42,0.04)', border: '1px solid rgba(200,146,42,0.2)', borderRadius: '8px', marginBottom: '12px' }}>
                <div style={{ fontFamily: 'var(--font-sc)', fontSize: '0.5625rem', letterSpacing: '0.14em', color: 'var(--gold-dk)', marginBottom: '8px' }}>YOUR AVATAR</div>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9375rem', fontStyle: 'italic', color: 'var(--text-muted)', lineHeight: 1.7 }}>{avatarFinal}</p>
              </div>
              <button onClick={startEditAvatar} style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontStyle: 'italic', color: 'rgba(15,21,35,0.45)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                Edit avatar ↗
              </button>
            </div>
          )}
      ...
      {step === 'score' && (
        <div>
          ...
          <textarea
            ...
            rows={4}
            style={{ width: '100%', marginTop: '16px', padding: '12px 14px', fontFamily: 'var(--font-body)', fontSize: '1rem', color: 'var(--text-meta)', background: 'rgba(200,146,42,0.02)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '8px', outline: 'none', resize: 'vertical', lineHeight: 1.65 }}
          />
          ...
          {scoreLocked && (
            <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <button onClick={() => { setScoreLocked(false) }} style={{ fontFamily: 'var(--font-body)', fontSize: '0.8125rem', fontStyle: 'italic', color: 'rgba(15,21,35,0.4)', background: 'none', border: 'none', cursor: 'pointer', marginLeft: '8px' }}>Edit</button>
            </div>
          )}
        </div>
      )}
          ...
          {!horizonLocked || step === 'horizon' ? (
            <>
              ...
              {horizonMsgs.length === 0 && (
                <div style={{ padding: '16px 18px', background: 'rgba(200,146,42,0.04)', border: '1px solid rgba(200,146,42,0.18)', borderLeft: '3px solid rgba(200,146,42,0.4)', borderRadius: '10px', marginBottom: '16px' }}>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9375rem', fontWeight: 300, color: 'var(--text-meta)', lineHeight: 1.75, margin: 0 }}>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9375rem', color: 'var(--text-meta)', lineHeight: 1.75, marginBottom: '8px' }}>
                If the genie granted your wish in {domain.label}, what would it be?
              </p>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.8125rem', fontStyle: 'italic', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: '16px' }}>
              ...
              <textarea
                ...
                rows={3}
                style={{ width: '100%', padding: '12px 14px', fontFamily: 'var(--font-body)', fontSize: '1rem', color: 'var(--text-meta)', background: 'rgba(200,146,42,0.02)', border: '1px solid rgba(200,146,42,0.25)', borderRadius: '8px', outline: 'none', resize: 'vertical', lineHeight: 1.65, marginBottom: '12px' }}
              />
            ...
            <div>
              <div style={{ padding: '14px 16px', background: 'rgba(200,146,42,0.04)', border: '1px solid rgba(200,146,42,0.2)', borderRadius: '8px', marginBottom: '10px' }}>
                <div style={{ fontFamily: 'var(--font-sc)', fontSize: '0.5625rem', letterSpacing: '0.14em', color: 'var(--gold-dk)', marginBottom: '6px' }}>YOUR HORIZON</div>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9375rem', fontStyle: 'italic', color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: '8px' }}>{horizonText}</p>
              ...
              <button onClick={() => { setHorizonLocked(false); setStep('horizon') }} style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontStyle: 'italic', color: 'rgba(15,21,35,0.45)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                Edit →
              </button>
            </div>
          )}
          ...
          return (
            <div key={d.id} style={{ padding: '8px 0', borderBottom: '1px solid rgba(200,146,42,0.07)', background: isFocus ? 'rgba(200,146,42,0.03)' : 'transparent', paddingLeft: isFocus ? '8px' : 0, borderLeft: isFocus ? '2px solid rgba(200,146,42,0.4)' : 'none', marginLeft: isFocus ? '-8px' : 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
                <span style={{ fontFamily: 'var(--font-sc)', fontSize: '0.6875rem', letterSpacing: '0.06em', color: isFocus ? 'var(--gold-dk)' : 'var(--text)', minWidth: '90px' }}>{isFocus ? '▸ ' : ''}{d.label}</span>
              ...
              {isFocus && <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.6875rem', fontStyle: 'italic', color: 'var(--text-muted)' }}>{d.question}</div>}
            </div>
          )
        })}
      </div>
      ...
      {mapData?.life_horizon_draft && (
        <div style={{ padding: '20px 28px', borderTop: '1px solid rgba(200,146,42,0.12)' }}>
          ...
          <textarea value={horizonText} onChange={e => setHorizonText(e.target.value)} disabled={horizonLocked}
            ...
            rows={4} style={{ width: '100%', padding: '12px 14px', fontFamily: 'var(--font-body)', fontSize: '1rem', fontStyle: 'italic', fontWeight: 300, color: 'var(--text-meta)', background: '#FFFFFF', border: horizonLocked ? '1px solid rgba(200,146,42,0.3)' : '1.5px dashed rgba(200,146,42,0.4)', borderRadius: '10px', resize: 'vertical', outline: 'none', lineHeight: 1.7, marginBottom: '8px', opacity: horizonLocked ? 0.7 : 1 }}
          />
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(15,21,35,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: '#FAFAF7', border: '1.5px solid rgba(200,146,42,0.78)', borderRadius: '14px', padding: '36px 32px', maxWidth: '380px', width: '100%', textAlign: 'center' }}>
        <span style={{ fontFamily: 'var(--font-sc)', fontSize: '11px', letterSpacing: '0.2em', color: 'var(--gold-dk)', display: 'block', marginBottom: '14px' }}>THE MAP</span>
        <h2 style={{ fontFamily: 'var(--font-sc)', fontSize: '22px', fontWeight: 400, color: 'var(--text)', marginBottom: '10px' }}>Sign in to begin.</h2>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9375rem', fontWeight: 300, color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: '24px' }}>
          The Map saves your progress as you go — you can pick up where you left off, any time.
        </p>
        <a href={`/login?redirect=${returnUrl}`} style={{ display: 'block', padding: '14px', borderRadius: '40px', border: '1.5px solid rgba(200,146,42,0.78)', background: 'rgba(200,146,42,0.05)', fontFamily: 'var(--font-sc)', fontSize: '0.9375rem', letterSpacing: '0.16em', color: 'var(--gold-dk)', textDecoration: 'none' }}>
  ...
  return (
    <div className="page-shell">
      ...
      <div className="tool-wrap">
        ...
        {/* Mapping phase — wheel + active domain */}
        {phase === 'mapping' && (
          <div>
            {/* Two-column layout: wheel + domain step */}
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px,460px) 1fr', gap: '32px', alignItems: 'start', marginBottom: '24px' }}>
              ...
              <div>
                ...
                {/* All complete CTA */}
                {allComplete && (
                  <div style={{ marginTop: '20px', padding: '20px 22px', background: 'rgba(200,146,42,0.05)', border: '1.5px solid rgba(200,146,42,0.78)', borderRadius: '12px', textAlign: 'center' }}>
                    <p style={{ fontFamily: 'var(--font-sc)', fontSize: '0.75rem', letterSpacing: '0.14em', color: 'var(--gold-dk)', marginBottom: '6px' }}>ALL SEVEN DOMAINS COMPLETE</p>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontStyle: 'italic', color: 'var(--text-muted)', marginBottom: '14px' }}>
              ...
              <div>
                {activeDomain ? (
                  ...
                  <div style={{ padding: '32px 24px', background: 'rgba(200,146,42,0.02)', border: '1px solid rgba(200,146,42,0.15)', borderRadius: '12px', textAlign: 'center' }}>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', fontStyle: 'italic', color: 'var(--text-muted)', lineHeight: 1.7 }}>
        ...
        {phase === 'synthesis' && (
          <div style={{ animation: 'fadeUp 0.4s ease-out' }}>
            <div style={{ padding: '32px 28px', background: '#FFFFFF', border: '1px solid rgba(200,146,42,0.2)', borderRadius: '12px', textAlign: 'center' }}>
              {thinking ? (
                <>
                  <p style={{ fontFamily: 'var(--font-sc)', fontSize: '0.75rem', letterSpacing: '0.14em', color: 'var(--gold-dk)', marginBottom: '12px' }}>BUILDING YOUR MAP</p>
                  <div className="typing-indicator"><span /><span /><span /></div>
                </>
              ) : synthesis}
            </div>
        ...
        {phase === 'results' && mapData && (
          <>
            <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={() => setPhase('mapping')} style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontStyle: 'italic', color: 'rgba(15,21,35,0.45)', background: 'none', border: 'none', cursor: 'pointer' }}>
                ← Go back and edit
              </button>
            </div>
            <ResultsCard
