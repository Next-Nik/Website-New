...
const inputStyle = {
  ...
  fontSize: '16px', color: '#0F1523', outline: 'none',
  ...
  return (
    <>
      ...
      <input type="email" value={email} onChange={e => setEmail(e.target.value)}
        ...
        style={{ ...inputStyle, fontFamily: "'Lora', Georgia, serif" }}
      />
      ...
      <div style={{ textAlign: 'right', marginTop: '-4px', marginBottom: '20px' }}>
        <button onClick={() => onSwitch('reset')} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', ...body, fontSize: '13px', color: 'rgba(15,21,35,0.45)', textDecoration: 'underline' }}>
          Forgot password?
        </button>
      </div>
      ...
      <p style={{ ...body, fontSize: '14px', color: 'rgba(15,21,35,0.45)', marginTop: '24px', textAlign: 'center' }}>
        New here?{' '}
        <button onClick={() => onSwitch('signup')} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', ...body, fontSize: '14px', color: '#A8721A', textDecoration: 'underline' }}>
          Create an account
        </button>
      </p>
    </>
...
function SignUpScreen({ onSwitch, onDone }) {
  ...
  async function handleSubmit() {
    ...
    // Sign them straight in — no need to wait for email confirmation
    // if Supabase "Confirm email" is disabled (recommended)
  ...
  return (
    <>
      ...
      <div style={{ margin: '4px 0 16px' }}>
        ...
        <Checkbox checked={mailing} onChange={() => setMailing(v => !v)}>
      ...
      <p style={{ ...body, fontSize: '14px', color: 'rgba(15,21,35,0.45)', marginTop: '24px', textAlign: 'center' }}>
        ...
        <button onClick={() => onSwitch('signin')} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', ...body, fontSize: '14px', color: '#A8721A', textDecoration: 'underline' }}>
          Sign in
        </button>
      </p>
    </>
...
function ResetScreen({ onSwitch }) {
  ...
  if (sent) return (
    <div style={{ textAlign: 'center' }}>
      ...
      <button onClick={() => onSwitch('signin')} style={{ marginTop: '24px', background: 'none', border: 'none', padding: 0, cursor: 'pointer', ...sc, fontSize: '13px', letterSpacing: '0.14em', color: 'rgba(15,21,35,0.45)', textDecoration: 'underline' }}>
        Back to sign in
      </button>
    </div>
  )
  ...
  return (
    <>
      <button onClick={() => onSwitch('signin')} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', ...sc, fontSize: '13px', letterSpacing: '0.14em', color: 'rgba(15,21,35,0.45)', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ← Back
      </button>
...
function GoogleButton() {
  async function handleGoogle() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/auth/callback?redirect=${encodeURIComponent(dest)}` }
    })
  }
  return (
...
function ErrorMsg({ children }) {
  return (
    <p style={{ ...body, fontSize: '15px', color: 'rgba(15,21,35,0.72)', marginTop: '8px', padding: '10px 14px', background: 'rgba(200,146,42,0.05)', borderRadius: '14px', border: '1.5px solid rgba(200,146,42,0.35)' }}>
      {children}
    </p>
  )
}
