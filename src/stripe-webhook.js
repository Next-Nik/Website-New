  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        // Prefer client_reference_id (set by Pricing.jsx) — grants access to the
        // logged-in account regardless of which billing email was used at checkout.
        let userId = session.client_reference_id || null

        if (!userId) {
          const email = session.customer_details?.email
          if (!email) { console.error('No email or client_reference_id on session', session.id); break }
