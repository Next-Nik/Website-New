  ...
  return (
    ...
    >
      ...
      {(() => {
        ...
        // Compute vertex positions + store for click targets
        ...
        const polyPoints = verts.map(v => `${v.x.toFixed(1)},${v.y.toFixed(1)}`).join(' ')
        ...
        return (
          <g style={{ opacity: bloomed ? 1 : bloomT }}>
            <polygon
              ...
              style={{ pointerEvents: 'none' }}
            />
            {/* Clickable vertex dots — same pattern as spoke tip dots */}
            {verts.map(v => v.scored && (
              <g
                key={`score-vert-${v.i}`}
              ...
              >
                ...
                <circle cx={v.x.toFixed(1)} cy={v.y.toFixed(1)} r={14} fill="transparent" />
                {/* Visible dot */}
                <circle
                  cx={v.x.toFixed(1)}
                  cy={v.y.toFixed(1)}
