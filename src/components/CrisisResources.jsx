...
export function CrisisResources({ variant = 'compact', defaultCountry = null }) {
  ...
  return (
    <div>
      ...
      <div style={{ borderTop: '1px solid rgba(76,107,69,0.20)', paddingTop: '20px' }}>
        <p style={{ ...sc, fontSize: '13px', letterSpacing: '0.16em', color: '#262420', marginBottom: '12px' }}>
          International
        </p>
      ...
      {otherCountries.length > 0 && (
        <div style={{ marginTop: '24px', borderTop: '1px solid rgba(76,107,69,0.10)', paddingTop: '16px' }}>
          <button
            ...
            <div style={{ marginTop: '16px' }}>
              {otherCountries.map(cc => (
                <div key={cc} style={{ marginBottom: '20px' }}>
                  <p style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', color: '#262420', marginBottom: '8px' }}>
                    {countryName(cc)}
                  </p>
...
function CountrySelect({ country, setCountry, resources }) {
  ...
  return (
    <div style={{ marginBottom: '20px' }}>
      <label style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', color: '#262420', display: 'block', marginBottom: '6px' }}>
        Your region
      </label>
      <select
        value={country}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px 16px', marginBottom: resource.description ? '8px' : 0 }}>
        {resource.phone && (
          <a href={`tel:${resource.phone.replace(/\s/g, '')}`}
            style={{ ...body, fontSize: '15px', color: '#262420', textDecoration: 'none', fontWeight: 500 }}>
            📞 {resource.phone}
          </a>
        )}
        {resource.sms && (
        ...
        {resource.web_url && (
          <a href={resource.web_url} target="_blank" rel="noopener noreferrer"
            style={{ ...body, fontSize: '14px', color: '#262420', textDecoration: 'none', borderBottom: '1px solid rgba(76,107,69,0.3)' }}>
            Online chat
          </a>
        )}
      </div>
      ...
      {orderedCodes.map(cc => (
        <div key={cc} style={{ marginBottom: '40px' }}>
          <h3 style={{ ...body, fontSize: '20px', fontWeight: 400, color: '#0F1523', marginBottom: '14px',
            paddingBottom: '8px', borderBottom: '1px solid rgba(76,107,69,0.20)' }}>
            {countryName(cc)}
            {cc === userCountry && (
              <span style={{ ...sc, fontSize: '13px', letterSpacing: '0.14em', color: '#262420', marginLeft: '12px' }}>
                YOUR REGION
              </span>
            )}
          </h3>
