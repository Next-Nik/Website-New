...
// Continuous spin — 22 seconds per full rotation, matches Earth intro
const SPIN_DPS = 360 / 22

// Animation durations (ms) — from the spec
const T_BLOOM_NODE  = 300   // each node blooms in
const T_BLOOM_STAG  = 50    // stagger between nodes
const T_PULL        = 240   // clicked node pulls to centre
const T_BREATHE     = 280   // pause before children appear
const T_RISE        = 280   // going back up

function pos(index, rotDeg) {
...
function sizing(lines) {
  const maxL = Math.max(...lines.map(l => l.length))
  return {
    fontSize: 20,
    radius: Math.round(46 + (lines.length - 1) * 9 + Math.max(0, maxL - 9) * 0.8),
    lh: 1.28,
  }
}

const SETTLED_STATES = Array.from({ length: N }, () => ({ sc: 1, op: 1, ox: 0, oy: 0 }))
const HIDDEN_STATES  = Array.from({ length: N }, () => ({ sc: 0, op: 0, ox: 0, oy: 0 }))
...
export default function Heptagon({
  ...
  const [nodeStates, setNodeStates] = useState(HIDDEN_STATES)

  // Re-bloom every time domains changes (level navigation)
  useEffect(() => {
    if (!domains?.length) return
    animStartRef.current = null
    drillIdxRef.current  = null
    setPhase('blooming')
    setNodeStates(HIDDEN_STATES)
  }, [domains])
  ...
  useEffect(() => {
    function tick(ts) {
      ...
      if (phase === 'blooming') {
        ...
        const tp  = pos(idx, rotRef.current)
        const ddx = CX - tp.x, ddy = CY - tp.y

        setNodeStates(Array.from({ length: N }, (_, i) => {
          if (i === idx) return { sc: 1 + te * .12, op: 1, ox: ddx * te, oy: ddy * te }
          const p = pos(i, rotRef.current)
        ...
        if (el >= T_BREATHE) {
          ...
          drillIdxRef.current = null
          onDrillDown?.(idx)
          // domains change → useEffect re-triggers bloom
        }
      }
        ...
        if (t >= 1) { setNodeStates(SETTLED_STATES); setPhase('settled') }
      }
  ...
  const polyP = Array.from({ length: N }, (_, i) => { const p = pos(i, rot); return `${p.x},${p.y}` }).join(' ')
  ...
  return (
    <svg className={styles.svg} viewBox="0 0 480 480" xmlns="http://www.w3.org/2000/svg" aria-label="NextUs Seven Domains Heptagon">
      ...
      {Array.from({length:N},(_,i)=>{ const p=pos(i,rot); return <line key={i} x1={CX} y1={CY} x2={p.x} y2={p.y} stroke="rgba(200,146,42,0.08)" strokeWidth="0.5"/> })}

      {(domains||[]).map((d,i)=>{
        const ns = nodeStates[i]||{sc:0,op:0,ox:0,oy:0}
        const p  = pos(i, rot)
        const active = !busy && !isIdle && i===activeIndex
        const words  = getLabel(d.name)
        const {fontSize,radius:r,lh} = sizing(words)
        ...
        return (
          <g key={d.id||i}
            style={{ cursor:busy?'default':'pointer', opacity:ns.op, transform:`translate(${ns.ox}px,${ns.oy}px) scale(${ns.sc})`, transformOrigin:`${p.x}px ${p.y}px`, outline:'none' }}
      ...
      <g className={styles.centreGroup} onClick={onCentreClick}
        ...
        aria-label={centreLabel?`${centreLabel} — tap to go back`:'Our Planet'}
        ...
        {centreLabel&&(
          ...
          >
            {centreLabel.split(' ').map((w,wi,a)=>(
              <tspan key={wi} x={CX} dy={wi===0?`${-(a.length-1)*.6}em`:'1.25em'}>{w}</tspan>
            ))}
          </text>
        )}
      </g>
